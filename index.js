/*
  Project: Al-Quds
  Theme: Memory, Witness, Resistance, Dignity
*/
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  overlay.addEventListener("click", function() {
    ...
    gsapWithCSS.to(".overlay", 2, {
      clipPath: "polygon(0% 0%, 100% 0%, 100% 0%, 0% 0%)",
      ease: "power4.inOut"
    });
    gsapWithCSS.to(".loader__img", 2, {
...
// === WATERY / DIFFUSION CURSOR REVEAL ===

const overlay = document.querySelector(".overlay");
const reveal = document.querySelector(".overlay-reveal");

const trail = [];
const MAX_POINTS = 20; // more = more fluid
...
if (overlay && reveal) {
  ...
  function animate() {
    ...
    // add point to trail
    trail.unshift({ x: currentX, y: currentY, life: 1 });

    if (trail.length > MAX_POINTS) trail.pop();

    // decay trail
    trail.forEach(p => p.life *= 0.95);

    // build layered mask
    const masks = trail.map(p =>
      `radial-gradient(circle ${140 * p.life}px at ${p.x}px ${p.y}px, 
        rgba(255,255,255,${p.life}) 0%, 
        rgba(255,255,255,0) 70%)`
    );

    reveal.style.setProperty("--mask", masks.join(","));
